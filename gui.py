...
def draw_tree(main_display, tree_root):
    max_width = get_max_width(tree_root)
    ...
    def draw_tree_helper(tree, index, depth):
        if tree is None:
            return
        width = 1 << (depth - 1)
        ...
        line_thickness = 0 if tree.is_selected() else 2
        pygame.draw.circle(main_display, BLACK, (x, y), RADIUS, width=line_thickness)
        place_text(main_display, (x - MIN_MARGIN, y - MIN_MARGIN), str(tree.value))
        left_child_pos = draw_tree_helper(tree.get_left(), 2 * index, depth + 1)
        if left_child_pos is not None:
            pygame.draw.line(main_display, RED, (x, y), left_child_pos)
        right_child_pos = draw_tree_helper(tree.get_right(), 2 * index + 1, depth + 1)
        if right_child_pos is not None:
            pygame.draw.line(main_display, RED, (x, y), right_child_pos)
        return (x, y)

    draw_tree_helper(tree_root, 0, 1)
...
def get_max_width(tree_root):
    return 1 << (tree_root.height())
